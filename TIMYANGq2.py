...
#print ("The rectangle has corners A(%i,%i), B(%i,%i), C(%i,%i), D(%i,%i)." %(Ax,Ay,Bx,By,Cx,Cy,Dx,Dy))
#Px = int(input("Please enter the X-Coordinate of your point: "))
#Py = int(input("Please enter the Y-Coordinate of your point: "))
#if Ax <= Px <= Cx and Ay <= Py <= Cy:
    #print ("Your coordinates lies within this rectangle.")
#else:
    #print ("Your coordinates dose not lie within this rectangle.")


#Level 3+
#import math

#def XYSeperation(ans):
    #x = int(ans[1:ans.find(",")])
    #y = int(ans[ans.find(",") + 1:ans.find(")")])
    #return x, y
...
#print ("The rectangle has corners A(%i,%i), B(%i,%i), C(%i,%i), D(%i,%i)." %(Ax,Ay,Bx,By,Cx,Cy,Dx,Dy))
#PXY = input("Please enter the X and Y coordinates of your point in form (x,y): ")
#Px, Py = XYSeperation(PXY)
#if Ax <= Px <= Cx and Ay <= Py <= Cy:
    #print ("Your coordinates lies within this rectangle.")
#else:
    #print ("Your coordinates dose not lie within this rectangle.")
...
#def XYVSeperation(ans):
    #string = ans.replace(" ","")
    #x = int(string[1:string.find(",")])
    #y = int(string[string.find(",") + 1:string.find(")")])
    #v = string[string.find(")") + 1::]
    #return x, y, v
...
#print ("The rectangle has corners A(%i,%i), B(%i,%i), C(%i,%i), D(%i,%i)." %(Ax,Ay,Bx,By,Cx,Cy,Dx,Dy))
#PXV = input("Please enter the X and Y coordinates of your point, followed by a vertices of your choice in the form (x,y)V (V being either A, B, C or D): ")
#Px, Py ,V = XYVSeperation(PXV)
#if Ax <= Px <= Cx and Ay <= Py <= Cy:
    #print ("Your coordinates lies within this rectangle.")
    #if V == "A":
        #Vx = Ax
        ...
        #Vy = Dy
    #else:
        #print ("However, your choice of a vertice was neither A, B, C nor D. Please try again.")
        #pass1 = False
    #if pass1 == True:
        #finalDistance = pointsDistance(Px, Py, Vx, Vy)
        #print ("The point of your choice (%i,%i), is %0.4f units away from vertice %s." %(Px, Py, finalDistance, V))
#else:
    #print ("Your coordinates dose not lie within this rectangle.")
...
#def pointsDistance(x1, y1, x2, y2):
    #d = float(sqrt((x2 - x1)**2 + (y2 - y1)**2))
    #return d
#def findClosestVertice(x1, y1):
    #A = pointsDistance(x1, y1, Ax, Ay)
    #B = pointsDistance(x1, y1, Bx, By)
    #C = pointsDistance(x1, y1, Cx, Cy)
    #D = pointsDistance(x1, y1, Dx, Dy)
    #if A > B or A > B or A > C or A > D:
        #if B > C or B > D:
            #if C > D:
                #ans = "D"
            #else:
...
#print ("The rectangle has corners A(%i,%i), B(%i,%i), C(%i,%i), D(%i,%i)." %(Ax,Ay,Bx,By,Cx,Cy,Dx,Dy))
#PXV = input("Please enter the X and Y coordinates of your point, followed by a vertices of your choice in the form (x,y)V (V being either \"A, B, C, D or V\". Inputting V will result in the return of the closest vertice to your point): ")
#Px, Py ,V = XYVSeperation(PXV)
#if Ax <= Px <= Cx and Ay <= Py <= Cy:
    #print ("Your coordinates lies within this rectangle.")
    #if V == "A":
        ...
        #typeOfInput = 2
    #else:
        #print ("However, your choice of a vertice was neither A, B, C, D or V. Please try again.")
        #typeOfInput = 3
    #if typeOfInput == 1:
        #finalDistance = pointsDistance(Px, Py, Vx, Vy)
        #print ("The point of your choice (%i,%i), is %0.2f units away from vertice %s." %(Px, Py, finalDistance, V))
    #elif typeOfInput == 2:
        #closestVertice = findClosestVertice(Px, Py)
        #print ("The point of your choice (%i,%i), is closest to vertice %s." %(Px, Py, closestVertice))
#else:
    #print ("Your coordinates dose not lie within this rectangle.")
